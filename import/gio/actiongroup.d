...
struct GActionGroupInterface {
  ...
  /* virtual functions */
  gboolean              function(GActionGroup  *action_group,
                                                        const(gchar)*   action_name) has_action;

  gchar **              function(GActionGroup  *action_group) list_actions;

  gboolean              function(GActionGroup  *action_group,
                                                        const(gchar)*   action_name) get_action_enabled;

  const(GVariantType)*   function(GActionGroup  *action_group,
                                                        const(gchar)*   action_name) get_action_parameter_type;

  const(GVariantType)*   function(GActionGroup  *action_group,
                                                        const(gchar)*   action_name) get_action_state_type;

  GVariant *            function(GActionGroup  *action_group,
                                                        const(gchar)*   action_name) get_action_state_hint;

  GVariant *            function(GActionGroup  *action_group,
                                                        const(gchar)*   action_name) get_action_state;

  void                  function(GActionGroup  *action_group,
                                                        const(gchar)*   action_name,
                                                        GVariant      *value) change_action_state;

  void                  function(GActionGroup  *action_group,
                                                        const(gchar)*   action_name,
                                                        GVariant      *parameter) activate_action;

  /* signals */
  void                  function(GActionGroup  *action_group,
                                                        const(gchar)*   action_name) action_added;
  void                  function(GActionGroup  *action_group,
                                                        const(gchar)*   action_name) action_removed;
  void                  function(GActionGroup  *action_group,
                                                        const(gchar)*   action_name,
                                                        gboolean       enabled) action_enabled_changed;
  void                  function(GActionGroup   *action_group,
                                                        const(gchar)*    action_name,
                                                        GVariant       *state) action_state_changed;
  ...
  gboolean              function(GActionGroup        *action_group,
                                                        ...
                                                        GVariant           **state_hint,
                                                        GVariant           **state) query_action;
};

GType                   g_action_group_get_type                         () pure;
